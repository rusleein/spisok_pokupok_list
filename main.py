...
vvod = None
mis = ['-' * 30, 'Что-то пошло не так. Некорректные данные. Введите еще раз.', '-' * 30]
print('-' * 120)
...
def change():
    ...
    flag_done = False
    while not flag_done:  # Проверяем введеное значение
        done = input('На что хотите изменить?:')  # Получаем значение на которое будем менять
        if int(chto_menyaem) == 2 and is_number(done):
            flag_done = True
        elif int(chto_menyaem) == 3 and is_number(done):
            flag_done = True
        else:
            flag_done = False
            print(*mis, sep='\n')

    itog[int(stroka) - 1][int(chto_menyaem) - 1] = done
