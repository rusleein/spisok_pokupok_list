...
def pokupki():  # Функция для заполнения списка и продолжения заполнения
    ...
    global itog
    summa = 0
    while spisok != ['Итог'] or spisok != ['итог']:  # Пока введено не "ИТОГ" продолжаем работу
...
Введите "Итог" для вывода списка Ваших покупок:''').split()  # Получаем наименование, кол-во и цену
        ...
        if len(spisok) == 3 and is_number(spisok[1]) and is_number(spisok[2]) or spisok in [['Итог'], ['итог']]:
            ...
            if spisok == ['Итог'] or spisok == ['итог']:  # Если введено "ИТОГ", останавливаем цикл и выводим список
                break
...
while choice.lower() not in ['стоп']:
    if choice.lower() in ['продолжить']:
        ...
        delete()
    elif choice.lower not in ['стоп', 'продолжить', 'изменить', 'удалить']:
        print('- ' * 30)
