...
Введите "Итог" для вывода списка Ваших покупок:''').split()  # Получаем наименование, кол-во и цену
    ...
    a = 0
    for i in itogg.items():
        a += 1
        i[1].append(round(float(i[1][0]) * float(i[1][1]), 2))  # Сумма товара (кол-во * цену)
        summa += i[1][2]  # Итоговая сумма
        if not is_int(i[1][0]):  # Если кол-во НЕ целое число, то будет КГ., если целое, то ШТ.
            i[1][0] = str(round(float(i[1][0]), 2)) + ' кг.'
        else:
            i[1][0] = str(i[1][0]) + ' шт.'
        i[1][1] = str(i[1][1]) + ' руб.'  # добавляем приписку ".руб" к итоговому списку
        i[1][2] = str(i[1][2]) + ' руб.'  # добавляем приписку ".руб" к итоговому списку
        vvod.add_row([a, i[0], *i[1]])  # Добавляем строку в итоговый список
